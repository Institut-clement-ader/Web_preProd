<?php
// $Id: report.php 2822 2014-03-03 17:38:31Z cimorrison $
  ...
  foreach ($field_order_list as $field)
  {
    ...
    switch ($field)
    {
      case 'end_time':
		...
		// MODIF PABLO //
		// Duration field : vrai duree, sans la nuit !!!
		$duree_travail = $eveningends - $morningstarts;
		$duree_travail += ($eveningends_minutes - $morningstarts_minutes)/60;
		...
		$nb_jours = floor($d['duration']/24);
		$duree_vraie = $d['duration'] - $nb_jours *(24-$duree_travail);
        // $d_string = $d['duration'] . ' ' . $d['dur_units'];
		$d_string = $duree_vraie . ' ' . $d['dur_units'];
                    ...
                    &$room_hash, &$name_hash)
{
  ...
  {
    $duree_totale = (min((int)$row['end_time'], $report_end) -
                  max((int)$row['start_time'], $report_start)) / SECONDS_PER_HOUR;
	$nb_jours = floor($duree_totale/24);
	$increment = $duree_totale - $nb_jours *(24-$duree_travail);
