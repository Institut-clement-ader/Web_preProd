...
function create_field_entry_rooms($disabled=FALSE)
{
  ...
  {
    echo "<span> <i>" . get_vocab("ctrl_click") . "</i> </span>\n";
  }
  echo "</div>\n";
...
function create_field_entry_type($disabled=FALSE)
{
  ...
  $explications = "<i> Veuillez préciser l'établissement de l'utilisateur pour connaître au mieux le taux d'occupation sur les moyens dont les frais de maintenance sont partagés. </i>";
...
function create_field_entry_custom_field($field, $key, $disabled=FALSE)
{
  ...
  if ($params['label'] == 'Etablissement:')
  {
    $params['options'] = array('CRITT', 'EMAC', 'INSA', 'ISAE', 'UPS', 'Autre');
//	$params['disabled'] = array(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE);
//	$params['create_hidden'] = array(TRUE, FALSE, FALSE, FALSE, FALSE, FALSE);
//    $params['value'] = 'rien';
    generate_radio_group($params);
  }
  ...
  {
    generate_checkbox($params);
	if ($params['label'] == 'Conditions:')
	{
	$cond = "<b> En cochant cette case, je confirme que j'ai contacté le(s) respo(s) d'équipement pour m'assurer de la faisabilité de mes essais. </b> ";
	//   echo "<div id=\"dwm_avertissement\" class=\"screenonly\">\n";
    echo $cond ;
	}
  }
  ...
  else
  {
    ...
    generate_input($params);
	if ($params['label'] == 'Encadrant:')
	{
	$explic = "<i> Veuillez donner le nom de votre encadrant principal si vous êtes doctorant ou post-doctorant. </i> ";
	//   echo "<div id=\"dwm_avertissement\" class=\"screenonly\">\n";
    echo $explic ;
	}
  }
