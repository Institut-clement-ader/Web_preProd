...
if ($eeSettings['eeUploadDir']) {
	...
	$eeOutput .= '<form action="' . $eeAction . '" method="post" enctype="multipart/form-data" name="eeUploadForm" id="eeUploadForm">
		...
		$eeOutput .= '<h2>Téléverser des fichiers</h2>';
		...
		<p class="sfl_instuctions">Taille maximale : ' . $eeSettings['ee_upload_max_filesize'] . ' Mb par fichier. Pour sélectionner plusieurs fichiers, restez appuyé sur Ctrl en sélectionnant les fichiers (Cmd pour Mac)</p>
				...
				document.getElementById("eeFileInput").addEventListener("change", function(){
				    ...
				    for(var i = 0; i<this.files.length; i++){
				        ...
				        if(eeFormatsArray.indexOf(eeExtension) == -1) {
					        eeError = "Cette extension (" + eeExtension + ") n\'est pas autorisée.";
				        }
			        ...
			        if (eeXhr.readyState == 4) { // && eeXhr.status == 200 <-- Windows returns 404?
			            ...
			            if(eeUploaded == eeFileCount) {
				            ...
				            if(eeXhr.responseText == "SUCCESS") {
						    	...
						    	if(n === 0) {
							    	alert("Erreur lors du téléversement de : " + eeFile.name);
		...
		<span id="eeUploadingNow"><img src="' . $eePluginURL . 'images/spinner.gif" width="32" height="32" alt="Spinner Icon" />Téléversement...</span>
		
		<button type="button" name="eeUploadGo" id="eeUploadGo" onclick="eeUploadProcessor(eeFileObjects);">Téléverser</button>
