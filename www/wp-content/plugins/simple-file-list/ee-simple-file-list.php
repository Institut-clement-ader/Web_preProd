...
function eeUpload($eeSettings) {
	...
	if (in_array($ext, $eeFormatsArray)) {
		...
		if (@move_uploaded_file($_FILES['eefile']['tmp_name'], $eeTargetFile)) { // Move the file to the final destination
			...
			if(!$eeAdmin) {
				?><script>alert('Fichier(s) téléversé(s)');</script><?php
			}
			...
			?><script>alert('Téléversement échoué!');</script><?php
...
function ee_main_page_display() {
	...
	if(!empty($eeSettings)) {
		...
		if(@$_POST['eeUpload'] AND check_admin_referer( 'ee-simple-file-list-upload', 'ee-simple-file-list-upload-nonce')) {
			...
			if(eeUpload($eeSettings)) {
				$eeMessages[] = "Fichier(s) téléversé(s).";
			}
	...
	if($eeAdmin) {
		...
		} elseif($eeSettings['eeAllowUploads'] == 'No') {
			
			include($eePluginPath . '/ee-upload-multi-display.php'); // If no uploader on front-side, default to multi-uploader on back-side - 11.06.17
		}
